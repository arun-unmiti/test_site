...
export class SendEmailComponent implements OnInit {
  ...
  constructor(private core: CoreService, private userService: UserDetailService, private route: Router, private activeRoute: ActivatedRoute) {}

  ngOnInit(): void {
    if (this.route.url.includes('rabi')) {
      this.season = ['1'];
    ...
    this.activeRoute.params.subscribe((res: any) => {
      if (res.client_id) {
        this.selectedClient = res.client_id;
      }
      this.getFilterData();
    })
  }
  ...
  getFilterData() {
    this.core.mail_post({purpose: 'get_lookups'}).then((response: any) => {
  ...
  setInputData(res: any) {
    this.clientData = res?.clients || [];
    ...
    // lkp_tehsil mapping
    for (let indx = 0; indx < res.tehsils.length; indx++) {
      const item = res?.tehsils[indx];
      this.tehsilMap.set(item.tehsil_id, item.tehsil_name);
    }

    // lkp_block mapping
    for (let indx = 0; indx < res.blocks.length; indx++) {
      const item = res.blocks[indx];
      this.blockMap.set(item.block_id, item.block_name);
    }

    // lkp_grampanchayats mapping
    for (let indx = 0; indx < res.grampanchayats.length; indx++) {
      const item = res.grampanchayats[indx];
      this.grampanchayatMap.set(item.grampanchayat_id, item.grampanchayat_name);
    }

    // lkp_village mapping
    for (let indx = 0; indx < res.villages.length; indx++) {
      const item = res.villages[indx];
      this.villageMap.set(item.village_id, item.village_name);
    }
    for (let i = 0; i < res.crops.length; i++) {
      const crop = res.crops[i];
      this.cropMap.set(+crop.crop_code, crop.crop_name)
    }
    for (let i = 0; i < res.notified_unit.length; i++) {
      const notifiedUnit = res.notified_unit[i];
      this.notifiedMap.set(notifiedUnit.notified_id, notifiedUnit.notified_unit_name)
    }
    for (let i = 0; i < res.year.length; i++) {
      const year = res.year[i];
      this.yearMap.set(year.id, year.year)
    }
    for (let i = 0; i < res.season.length; i++) {
      const season = res.season[i];
  ...
  async getPlanandCCEData() {
    ...
    const clients: any[] = this.selectedClient ? [this.selectedClient] : this.clientData.map((d: any) => d.UNIT_ID);
    let distIds: any[] = [];
    let states: any[] = [];
    await this.core.mail_post({purpose: 'get_all_client_district', client_id: clients, year: this.year, season: this.season}).then((res: any) => {
      if (res.status ==1 ) {
    ...
    Promise.all(promises.map((d) => this.core.mail_post(d)))
      .then((responses: any) => {
  ...
  if (this.exportType === 'excel') {
    ...
    this.message = 'Sending e-mail with calculated data, Please wait...';
     this.core.mail_post({...jsonData, purpose: 'generate_email'}, 'generate_email').then((response: any) => {
